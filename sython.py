...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@SOERCA_OFFLINE "))
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك حلم ابشر  @M_2_X')
...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@M_2_X"))
    except BaseException:
        pass
...
async def _(event):
      await event.reply("""السـورس يعمـل |ماكس
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
